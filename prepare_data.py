...
from utils.audio_transform import load_audio, acoustic_features
from utils.audio_transform import audio_augmentation
...
exp = './data'
configuration = {'step': 0, 'snr': [0], 'sampling_rate': 44100, 'hop_length': 1764, 'window_length': 256, 'fps': 25,
                 'normalization': 'interval', 'rng_pos': [-0.9, 0.9]}
...
def main():
    ...
    for i in range(len(file_list)):
        ...
        samplingRate = configuration['sampling_rate']
        audio_wave, sr = load_audio(soundPath, samplingRate)
        audiodata, keep = input_loader(audio_wave, start_pos, end_pos, configuration)
        acoustic_feats=None
        if keep:
            acoustic_feats, _ = acoustic_features(path=soundPath, start_pos=start_pos, end_pos=end_pos)
        ...
        if not keep:
            ...
            if os.path.exists(item):
                os.remove(item)
            h5file = '{}f_{:03d}.h5'.format(os.path.join(prefix, type), i)
            with h5py.File(h5file, 'a') as f:
                f.create_dataset('sound_path', data=soundPath)
                f.create_dataset('motion', data=motion)
                f.create_dataset('input', data=audiodata)
                f.create_dataset('acoustic_features', data=acoustic_feats)
    ...
    folders = glob.glob(join)
    print(join)
    print("in join, {} files were found".format(len(folders)))
    for i in range(len(folders)):
        path = folders[i]
        print('Using ', path)
        motion, start_position, end_position = output_loader(path)
